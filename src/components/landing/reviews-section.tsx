...
const reviews = [
  ...
  },
  {
    ...
    avatarQuery:
      "man contemplative peaceful nature background professional headshot",
    rating: 5,
  ...
  },
  {
    ...
    location: "Kent, UK",
    avatarQuery:
      "older woman serene thoughtful expression professional headshot",
    rating: 5,
      ...
      <div className="container mx-auto px-4 md:px-6 max-w-(--breakpoint-lg) space-y-16 md:space-y-20">
        <div className="text-center space-y-4 md:space-y-5 animate-fade-in-up">
          <div className="inline-flex items-center gap-2 md:gap-3 bg-vos-accent/10 text-vos-accent border-2 border-vos-accent/40 px-3 py-1.5 md:px-5 md:py-2 rounded-full text-xs sm:text-sm md:text-base font-semibold mb-2 md:mb-3 shadow-soft hover:shadow-glow_accent transition-shadow duration-300 max-w-[95%] mx-auto">
            <Users size={16} className="md:size-[18px] flex-shrink-0" />
          ...
          <p className="text-lg md:text-xl text-vos-secondary max-w-3xl mx-auto leading-relaxed">
            You are not alone. Discover how Voice of the Shepherd guides others
            ...
            >
              <CardHeader className="p-6 md:p-7 pb-4">
                <div className="flex items-center gap-4 mb-4">
                  <Image
                    src={`/abstract-geometric-shapes.png?height=64&width=64&query=${review.avatarQuery.replace(
                      /\s/g,
                      "+"
                    )}`}
                    alt={review.name}
                  ...
                  />
                  <div>
                    <h4 className="font-serif text-lg font-semibold text-vos-primary">
                      {review.name}
                    </h4>
                    <p className="text-xs text-vos-secondary/80 -mt-0.5">
                      {review.location}
                    </p>
                    <StarRating
              ...
              <CardContent className="p-6 md:p-7 pt-0 grow">
                <Quote
                  className="w-9 h-9 text-vos-accent/30 mb-2.5 -ml-1.5"
                  strokeWidth={1.25}
                />
        ...
        <div className="text-center pt-6 animate-fade-in-up animation-delay-600">
          <p className="text-lg text-vos-secondary">
            ...and{" "}
            <strong className="text-vos-primary font-semibold">
              many more
            </strong>{" "}
            experiencing daily blessings.{" "}
            <Link
              href="#reviews-all"
              className="text-vos-accent hover:underline font-semibold"
            >
