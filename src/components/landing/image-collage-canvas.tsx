...
const imageAssets: ImageAsset[] = [
  ...
  { src: "/abstract-spiritual-light.png", alt: "Ethereal light beams", baseWidth: 350, baseHeight: 500 },
  { src: "/celestial-sky-soft-clouds.png", alt: "Celestial sky", baseWidth: 500, baseHeight: 400 },
  { src: "/placeholder-osxcj.png", alt: "Illuminated scripture", baseWidth: 400, baseHeight: 600 },
]
...
export function ImageCollageCanvas() {
  ...
  const draw = useCallback((ctx: CanvasRenderingContext2D, canvas: HTMLCanvasElement) => {
    ...
    imagesRef.current.forEach((imgObj) => {
      if (imgObj.img.complete && imgObj.img.naturalHeight !== 0) {
        ...
        const baseScaleDim = Math.min(canvas.width, canvas.height) * imgObj.scale
        ...
        ctx.drawImage(imgObj.img, -drawWidth / 2, -drawHeight / 2, drawWidth, drawHeight)
        ctx.restore()
      }
    })
  }, [])
  ...
  const setupCanvasAndImages = useCallback(() => {
    ...
    if (imagesRef.current.length === 0 && loadedImagesCount.current === imageAssets.length) {
      imageAssets.forEach((asset) => {
        ...
        const existingImage = imagesRef.current.find((i) => i.img.src === img.src)
        if (!existingImage) {
    ...
    if (!animationFrameId.current && loadedImagesCount.current === imageAssets.length) {
      animationFrameId.current = requestAnimationFrame(animate)
    }
  }, [animate, draw])
